...
import static sample.chess.util.Converter.getFENFromModelPiece;
...
public final class FENBuilder {
     ...
     */
    public static Predicate<Field> hasNotMoved = f -> f.getPiece().isPresent()
            && !f.getPiece().get().hasMoved();

    /*
     * returns true if field contains a rook eligible to castle.
     */
    public static Predicate<Field> rookOKToCastle = f -> hasNotMoved.test(f) && f.getPiece().orElse(null) instanceof Rook;

    /*
     * returns true if field contains a king eligible to castle.
     */
    public static Predicate<Field> kingOKToCastle = f -> hasNotMoved.test(f) && f.getPiece().orElse(null) instanceof King;

    /*
     * returns a String representation for the active player.
     */
    public static Function<Game, String> activePlayer = g -> g.getActivePlayer().getColor() == WHITE ? " w" : " b";
     ...
     */
    public static Function<Game, String> enpassant = Game::getEnpassantField;

    /*
     * returns a String representation of the half move and the full move clock.
     */
    public static Function<Game, String> count = g -> " " + g.getHalfMove()+ " " + g.getFullMove();
     ...
     *
     */
    public static Function<Game,String> boardToFENEmptyFieldsNotCounted = g -> Stream.of(g.getBoardState())
    ...
    public static Function<Game, StringBuilder> boardToFEN = g -> {
        ...
        //+= 9, by inserting /, the next insert has to be added at 9 and not 8.
        //we need to separate each row with a "/". 8 rows means 7 "/"'s.
     ...
     */
    public static Function<Game, String> gameToFEN = g -> boardToFEN.apply(g)
     ...
     */
    public static Function<Game, String> gameToFEN3FoldRule = g -> boardToFEN.apply(g)
            .append(castling.apply(g))
            .toString();
