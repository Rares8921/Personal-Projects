...
import static sample.chess.Game.attackingMoves;
import static sample.chess.Game.kingSave;
import static sample.chess.Game.searchForKings;
import static sample.chess.util.FENBuilder.rookOKToCastle;
...
public class King extends Piece {
    ...
    public Predicate<Game> notInCheck = g -> attackingMoves
    ...
    public BiPredicate<Game, Move> kingEligibleToCastle = (g, m) -> {
        BiPredicate<Move,Move> kingEligible = (m1, m2) -> !hasMoved()
                && notInCheck.test(g)
                && kingSave.test(g, m1)
                && kingSave.test(g, m2);
        if (m.getColor() == WHITE) {
    ...
    public BiPredicate<Game, Move> rookEligibleToCastle = (g, m) -> {
        if (m.getColor() == WHITE) {
            ...
            if (left.test(m)) {
                return !g.getPiece(0, 6).isPresent() && rookOKToCastle.test(g.getField(0, 7));
            }
        }
        if (m.getColor() == BLACK) {
            //long castling field next to Rook must be empty
            if (left.test(m)) {
                return !g.getPiece(7, 6).isPresent() && rookOKToCastle.test(g.getField(7, 7));
            }
            if (right.test(m)) {
