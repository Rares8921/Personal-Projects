...
import static sample.chess.util.Action.CASTLING;
import static sample.chess.util.Action.ENPASSANT;
import static sample.chess.util.Action.PROMOTION;
import static sample.chess.util.Converter.convert;
import static sample.chess.util.FENBuilder.gameToFEN;
import static sample.chess.util.FENBuilder.gameToFEN3FoldRule;
import static sample.chess.util.GameEvaluator.check;
import static sample.chess.util.GameEvaluator.checkAndFiftyMovesRule;
import static sample.chess.util.GameEvaluator.checkAndThreeFoldRepetitionRule;
import static sample.chess.util.GameEvaluator.fiftyMovesRule;
import static sample.chess.util.GameEvaluator.insufficientMaterial;
import static sample.chess.util.GameEvaluator.stalemate;
import static sample.chess.util.GameEvaluator.threeFoldRepetitionRule;
import static sample.chess.util.MoveValidator.kingSave;
import static sample.chess.util.MoveValidator.legalForMove;
import static sample.chess.util.MoveValidator.legalForPiece;
import static sample.chess.util.MoveValidator.pathUnobstructed;
import static sample.chess.util.MoveValidator.ValidationResult.LEGAL_MOVE;
import static sample.chess.util.Movement.left;
import static sample.chess.util.Movement.right;
import static sample.chess.util.Movement.twoColumns;
import static sample.chess.util.Movement.twoRows;
...
import sample.chess.pieces.Bishop;
import sample.chess.pieces.King;
import sample.chess.pieces.Knight;
import sample.chess.pieces.Pawn;
import sample.chess.pieces.Piece;
import sample.chess.pieces.Rook;
import sample.chess.util.Action;
import sample.chess.util.Converter;
import sample.chess.util.GameEvaluator;
import sample.chess.util.GameEvaluator.EvaluationResult;
import sample.chess.util.MoveValidator;
...
public class Game {
    ...
    private final List<String>                     threeFoldRepetionList = new ArrayList<>();
    private final ObservableList<String>           takenPieces           = FXCollections.observableList(new ArrayList<>());
    private final ObservableMap<Enum<Action>, Move> action                = FXCollections.observableMap(new HashMap<>());
    private final ObservableList<ValidationResult> validationResult      = FXCollections.observableList(new ArrayList<>());
    private final ObservableList<EvaluationResult> evaluationResult      = FXCollections.observableList(new ArrayList<>());
    ...
    private final Consumer<Move> evaluateGame = m -> evaluationResult.add(GameEvaluator
            .checkmate()
            .and(stalemate())
    ...
    private final Consumer<Move> validateMove = m -> validationResult.add(MoveValidator
            .legalForPlayer()
            .and(legalForMove())
            .and(legalForPiece())
     ...
     */
    private static final BiFunction<Game, Color, List<Field>> allOfColor = (g, c) -> Stream.of(g.getBoardState())
     ...
     */
    public static Function<Game, Map<Color,Field>> searchForKings = g -> Stream.of(g.getBoardState())
            .flatMap(Arrays::stream)
            .filter(Field::piecePresent)
            .filter(f -> f.getPiece().orElse(null) instanceof King)
            .collect(Collectors.toMap(f -> f.getPieceColor().orElse(null), f -> f));
    ...
    public static BiFunction<Game, Field, List<Move>> attackingMoves = (g, fieldUnderAttack) -> {
        ...
        return allOfColor.apply(g, fieldUnderAttack.getPieceColor().orElse(null) == WHITE ? BLACK : WHITE)
                .stream()
                .map(f -> {
                    testMove.setStart(f);
                    testMove.setPiece(f.getPiece().orElse(null));
                    return testMove;
                })
     ...
     */
    private final Consumer<Move> count = m -> {
     ...
     */
    public Predicate<Move> checkOnOpponent = m -> !attackingMoves.apply(this, m.getColor() == Color.WHITE ?
     ...
     */
    private final Consumer<Move> hasMoved = m -> {
     ...
     * can move to.
     */
    private final BiFunction<Game, Color, List<Field>> allPositionsFor = (g, c) -> Stream.of(g.getBoardState())
            .flatMap(Arrays::stream)
            .filter(f ->
                    !f.piecePresent()
                            || f.piecePresent()
                            && Objects.requireNonNull(f.getPiece().orElse(null)).getColor() == (c == Color.WHITE ? Color.BLACK : Color.WHITE))
            .collect(Collectors.toList());
     ...
     */
    private final BiFunction<Field, List<Field>, List<Move>> allMovesForPiece = (f, l) -> {
     ...
     */
    private final Function<Move, List<Move>> allMovesForOpponent = m -> {
     ...
     */
    public Predicate<Move> checkmate = m -> allMovesForOpponent.apply(m).isEmpty() && checkOnOpponent.test(m);

    /*
     * returns true if stalemate.
     */
    public Predicate<Move> stalemate = m -> allMovesForOpponent.apply(m).isEmpty() && !checkOnOpponent.test(m);
     ...
     */
    public Predicate<Move> insufficientMaterial = m -> insufficientMaterialColor.test(WHITE)
            && insufficientMaterialColor.test(BLACK);

    /*
     * returns true if the 50 moves rule is applicable.
     */
    public Supplier<Boolean> fiftyMoves = () -> halfMove >= 50;

    /*
     * returns true if the 3-fold repetition rule is applicable.
     */
    public Supplier<Boolean> threeFoldRepetition = () -> threeFoldRepetionList
            .stream()
     ...
     */
    private final Consumer<Move> setEnpassantField = m -> {
        List<String> columnLetters = Arrays.asList("a", "b", "c", "d", "e", "f", "g", "h");
        int ENPASSANT_ROW_WHITE = 3;
        int ENPASSANT_ROW_BLACK = 6;
     ...
     */
    private final Consumer<Move> enPassant = m ->  {
     ...
     */
    private final Consumer<Move> castling = m -> {
     ...
     */
    public Predicate<Move> isPromotion = m -> m.getPiece() instanceof Pawn
     ...
     */
    private final Consumer<Move> promotion = m -> {
    ...
    public Move viewToModel(Move move) {
        ...
        modelMove.setPiece(getPiece(move.getStartX(), move.getStartY()).orElse(null));
        return modelMove;
    }
    ...
    public Optional<GameState> getLastState() {
        return !gameStates.isEmpty() ? Optional.of(gameStates.get(gameStates.size() - 1)) : Optional.empty();
    }
