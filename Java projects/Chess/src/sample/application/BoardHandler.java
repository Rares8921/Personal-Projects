...
import sample.application.promotion.PromotionStage;
import sample.chess.Field;
...
import static sample.application.DragAndDropHandler.*;
import static sample.chess.util.Action.*;
import static sample.chess.util.Converter.convert;
import static sample.chess.util.Converter.viewPieceAtIndexes;
...
public class BoardHandler {
   ...
   private sample.application.Controller controller;
   private Label               viewField;
   private Field               modelField;
   private Timers              timers;

   //CONSTRUCTOR
   public BoardHandler(sample.application.Controller controller) {
   ...
   private void setDimensions() {
      ...
      minOfWidthAndHeight.addListener((ObservableValue, oldValue, newValue) -> board.getChildren().forEach(c -> c.styleProperty().set("-fx-font-size:" + adjustFontSize(newValue.intValue()) + ";")));
   ...
   public void set(Color piecesAtBottom) {
      ...
      board.getChildren()
      .forEach(c -> ((Label) c).setText(viewPieceAtIndexes.
   ...
   public BiConsumer<HBox, Color> setColor = (h,c) -> h.getChildren()
   .forEach(e -> {
   ...
   public void switchPlayer() {
      ...
      getTimers().ifPresent(Timers::switchTimers);
      setColor.accept(player1Active ? controller.player1Labels : controller.player2Labels, LIGHTGOLDENRODYELLOW);
   ...
   public void clear() {
      ...
      getTimers().ifPresent(Timers::stop);
   ...
   public void endGame() {
      ...
      getTimers().ifPresent(Timers::stop);
      setColor.accept(controller.player1Labels, LIGHTGOLDENRODYELLOW);
      setColor.accept(controller.player2Labels, LIGHTGOLDENRODYELLOW);
      controller.validationMessages.setText("");
   ...
   public void setDragAndDrop() {
      board.getChildren().forEach(c -> setOnDragDetected
   ...
   public void discoLights() {
      new Thread(() -> {
         for(int i=0; i<3; i++) {
            for(int j=0; j<80; j++){
               DropShadow dropShadow = new DropShadow(5.0, 3.0, 2.0, color(0.4, 0.4, 0.5));
               board.getChildren()
            ...
            .forEach(c -> {
               c.setEffect(null);
               ((Region) c).setBackground(((getRowIndex(c) + getColumnIndex(c)) % 2 == 0) ? lightTile : darkTile);
            });
         }
      }).start();
   }
