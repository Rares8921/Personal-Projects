...
public class Main extends Application {
    ...
    public void lee() {
        ...
        // If the start position was not selected, or the directions were not selected or if the start position was affected by other objects.
        boolean invalid = matr[iStart][jStart] != '_';
        for(int kk = 0; kk < 20 && !invalid; kk++) {
            if(matr[iStart][kk] == '/' || matr[kk][jStart] == '/' || matr[iStart][kk] == '+' || matr[kk][jStart] == '+') {
                invalid = true;
            }
        }
        if(iFinish == -1 || iStart == -1 || (!vertical && !horizontal && !diagonal && !knight) || cntTp % 2 != 0 || invalid) {
        ...
        try {
            ...
            // path animation
            final int[] k = {0};
            path = new Timeline(new KeyFrame(javafx.util.Duration.millis(30), e -> {
    ...
    public void start(Stage primaryStage) throws IOException {
        ...
        Image icon = new Image("/imgs/img.png");
            ...
            if(!algStarted) {
                ...
                if (x >= 0 && x <= 800 && y >= 41 && y <= 800) {
                    ...
                    if (e.getButton() == MouseButton.SECONDARY) { // if right click is pressed
                        ...
                        if (obstacles) {
                            // Check if current objects replaces the start/end position
                            if(i - 1 == iStart && j == jStart) {
                                Controller.staticStart.setText("-1, -1");
                            } else if(i - 1 == iFinish && j == jFinish) {
                                Controller.staticEnd.setText("-1, -1");
                            }
