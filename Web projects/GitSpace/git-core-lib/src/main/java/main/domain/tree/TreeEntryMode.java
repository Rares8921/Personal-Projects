    ...
    public TreeEntryMode {
        ...
        if (type.isPermissionless() && permission != 0) {
            throw new IllegalArgumentException(
                    "%s is permissionless but provided permissions: %o".formatted(type, permission)
            );
        }
    }

    /**
     * Formats the mode as an octal string without leading zeros.
     * Example: 100644 (file), 40000 (dir).
     */
