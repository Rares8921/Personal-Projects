...
class HorizonDetector:
    def __init__(self, exclusion_thresh: float, fov: float, acceptable_variance: float, frame_shape: tuple):
        """
        :param exclusion_thresh: how close horizon points have to be to predicted horizon in order to be considered valid
        :param fov: field of view of the camera
        :param acceptable_variance: minimum variance for horizon contour points
        :param frame_shape: used to convert exclusion_thresh from a pitch angle to pixels
        """
    ...
    def find_horizon(self, frame : np.ndarray, diagnostic_mode : bool = False):
        """
        :param frame:
        :param diagnostic_mode:
        :return: calculated values for horizon: roll, pitch, variance, is_good_horizon, mask
        """

        # If no horizon is found we should set default values
        ...
        edges = skimage.measure.block_reduce(edges, (KERNEL_SIZE, KERNEL_SIZE), np.max)
        ...
        # With the edge points we will know if sky is up
        x_abbr, y_abbr = [], []
        x_edge_points, y_edge_points = [], []
        ...
        max_points = 20
        step_size = len(x_edge_points) // max_points
        ...
        max_points = 100
        step_size = len(x_original) // max_points
        ...
        if self.predicted_roll is not None:
            ...
            angle_perpendicular = predicted_roll_radians + pi / 2
            ...
            if run != 0:
                ...
                predicted_b = y_perpendicular - predicted_m * x_perpendicular

            # 2 points on the line from the previous horizon
        ...
        # Filter out points that don't lie on the edge
        for idx, x_point in enumerate(x_abbr):
            y_point = y_abbr[idx]

            # If point is already on some edge
            if edges[y_point // KERNEL_SIZE][x_point // KERNEL_SIZE] == 0:
                continue

            # If there is no predicted horizon
            if self.predicted_roll is None:
                x_filtered.append(x_point)
                y_filtered.append(y_point)
                continue

            # If there is a predicted horizon, check if the current point is close to it
        ...
        if diagnostic_mode:
            ...
            if self.predicted_roll:
                ...
                cv2.putText(mask, 'Horizon Lock', (20, 40), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1,
        ...
        # Polynomial fit cu cele mai mici patrate
        m, b = np.polyfit(x_filtered, y_filtered, 1)
        roll = atan2(m, 1)
        roll = degrees(roll)

        # Sky direction (above/below)
        if m * avg_x + b > avg_y:
            sky_is_up = 1 # above
        else:
            sky_is_up = 0 # below
        ...
        # Take the dist from the center point of the image to the horizon and find the pitch in degress based on FOV and height
        # 2 points along horizon
        p1 = np.array([0, b])
        ...
        # Check if the plane is pointing above or below horizon
        ...
        # Variance will be treated as a confidence score
        p2_minus_p1 = p2 - p1
        distance_list = []
        ...
        # Adjust the roll in 0-360 degrees range
...
if __name__ == "__main__":
    ...
    EXCLUSION_THRESH = 3  # degrees of pitch above and below the horizon
    FOV = 48.8
    ...
    if is_good_horizon == 1 or is_good_horizon == True:
        normalized_roll = ((roll + 180) % 360) - 180

        M = cv2.getRotationMatrix2D((frame.shape[1] // 2, frame.shape[0] // 2), normalized_roll, 1.0)
        rotated_frame = cv2.warpAffine(frame, M, (frame.shape[1], frame.shape[0]),
                                       flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REPLICATE)
        cv2.putText(rotated_frame, f"Rotated by {normalized_roll:.2f} deg", (20, 40),
